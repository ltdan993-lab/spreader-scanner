...
// Compute historical volatility (annualized) from daily close prices
...
// Compute expected move from IV and DTE
// EM = price × IV × sqrt(DTE / 365)
export function computeExpectedMove(stockPrice, iv, dte) {
  return stockPrice * (iv / 100) * Math.sqrt(dte / 365)
}

// Compute IV rank from current IV and a range of historical IVs
// IV rank = (currentIV - 52wLow) / (52wHigh - 52wLow)
export function computeIVRank(currentIV, ivHistory) {
  ...
  if (high === low) return 50 // flat history, return midpoint
...
// ATM straddle price as market-priced expected move proxy
...
// Find best bull put spread from a filtered options chain
...
export function constructBullPutSpreads(puts, stockPrice, expectedMove, config) {
  ...
  // Filter to short leg candidates
  ...
  shortLegs.forEach(shortLeg => {
    // Find valid long legs (lower strike, same expiry)
    ...
    longLegs.forEach(longLeg => {
      ...
      const shortBidAskPct = shortLeg.ask > 0 ? (shortLeg.ask - shortLeg.bid) / ((shortLeg.ask + shortLeg.bid) / 2) : 1
      const profitTarget50 = credit * 0.5 // 50% profit target exit level
      ...
      spreads.push({
        ...
        returnOnRisk: maxLoss > 0 ? parseFloat(((credit / maxLoss) * 100).toFixed(1)) : 0,
      })
    })
  })
...
export function scoreVolatilityEdge(ivRank, ivRVRatio) {
  const rankScore = ivRank != null ? Math.min(ivRank / 80, 1) : 0
...
// Run all gates and return gate results + pass/fail
export function runAllGates(spread, stockPrice, ivRank, config = {}) {
  const gates = [
    ...
    gateIVRank(ivRank ?? 0, config.minIVRank ?? 30),
    ...
    gateExpectedMoveBuffer(spread.distToShort, spread.emRatio * spread.distToShort / (spread.emRatio || 1), config.minEMBuffer ?? 1.0),
